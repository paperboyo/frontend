...
const isValidDate = (date: mixed): boolean => {
...
const makeRelativeDate = (epoch: string | number, opts: Object = {}) => {
    let minutes;
    let hours;
    let days;
    ...
    const extendedFormatting = ['short', 'med'].includes(opts.format);

    if (!isValidDate(then)) {
        return false;
    }

    const delta = parseInt((now.getTime() - then) / 1000, 10);
...
const replaceLocaleTimestamps = (html: string | Document = document): void => {
...
const replaceValidTimestamps = (opts?: Object = {}): void => {
...
const initRelativeDates = (opts?: Object) => {
